...
class ruleBasedAI():
 ...
 # def _choose_valid_center(self, ind):
    #     dims = self.territory_map.dims
    #     habitat_quality = self.territory_map.habitat_quality
    #     diameter = self.territory_map.diameter

    #     territories = self.territory_map.get_territories()

    #     # Heuristic 2: budding for older males still associated with an existing territory.
    #     sex = self.pop[ind]["sex"]
    #     age = self.year - self.pop[ind]["year"] if hasattr(self, "year") else 0
    #     natal_territory = self.pop[ind]["territory"]

    #     if (
    #         sex == "male"
    #         and age > 0
    #         and natal_territory is not None
    #         and natal_territory in territories
    #     ):
    #         natal_center = np.array(territories[natal_territory]["center"])
    #         inner_radius = diameter / 2
    #         outer_radius = diameter * 0.85

    #         row_min = max(0, int(np.floor(natal_center[0] - outer_radius)))
    #         row_max = min(dims[0] - 1, int(np.ceil(natal_center[0] + outer_radius)))
    #         col_min = max(0, int(np.floor(natal_center[1] - outer_radius)))
    #         col_max = min(dims[1] - 1, int(np.ceil(natal_center[1] + outer_radius)))

    #         candidates = []
    #         for r in range(row_min, row_max + 1):
    #             for c in range(col_min, col_max + 1):
    #                 candidate = np.array([r, c])
    #                 radial_distance = np.linalg.norm(candidate - natal_center)
    #                 if radial_distance < inner_radius or radial_distance > outer_radius:
    #                     continue
    #                 if habitat_quality[r, c] <= 0:
    #                     continue

    #                 # Prefer good habitat and points just outside natal territory edge.
    #                 edge_penalty = abs(radial_distance - inner_radius) / max(outer_radius, 1e-9)
    #                 score = (0.85 * habitat_quality[r, c]) + (0.15 * (1.0 - edge_penalty))
    #                 candidates.append((score, (int(r), int(c))))

    #         if len(candidates) > 0:
    #             candidates.sort(key=lambda x: x[0], reverse=True)
    #             return candidates[0][1]

    #     # Heuristic 1: global centrality + quality strategy.
    #     coordinates = np.stack(np.indices(dims), axis=-1).reshape(-1, 2)
    #     land_mask = habitat_quality[coordinates[:, 0], coordinates[:, 1]] > 0
    #     candidates = coordinates[land_mask]

    #     if len(candidates) == 0:
    #         return None

    #     quality_vals = habitat_quality[candidates[:, 0], candidates[:, 1]]
    #     map_center = np.array([(dims[0] - 1) / 2.0, (dims[1] - 1) / 2.0])
    #     dists_to_center = np.linalg.norm(candidates - map_center, axis=1)
    #     max_center_dist = np.linalg.norm(np.array([dims[0] - 1, dims[1] - 1]) - map_center)
    #     norm_center_dist = dists_to_center / max(max_center_dist, 1e-9)

    #     # Strongly favor high-quality points, with a centrality bonus.
    #     score = (0.80 * quality_vals) + (0.20 * (1.0 - norm_center_dist))
    #     best_idx = int(np.argmax(score))
    #     r, c = candidates[best_idx]
    #     return (int(r), int(c))
